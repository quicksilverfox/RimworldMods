    ...
    class AnimalsFightBack
    {
        ...
        public static class Pawn_MindState_Notify_DamageTaken_Patch
        {
            ...
            static void Postfix(ref DamageInfo __state, ref Pawn_MindState __instance)
            {
                ...
                if (defender.CurJob != null && (defender.CurJob.def == JobDefOf.Flee || defender.CurJob.def == JobDefOf.FleeAndCower || defender.CurJob.def == JobDefOf.AttackMelee || defender.CurJob.def == JobDefOf.AttackStatic || defender.CurJob.def == JobDefOf.PredatorHunt))
                    return;
                ...
                if (!fight)
                {
                    float powerRatio = (defender.kindDef.combatPower * defender.health.summaryHealth.SummaryHealthPercent * defender.ageTracker.CurLifeStage.bodySizeFactor)
